	...
	*/
	...
	function &get_posts() {
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses = $this->meta_query->get_sql( 'post', $wpdb->network_posts, 'ID', $this );
			// $join   .= $clauses['join'];
			// $where  .= $clauses['where'];
		...
		if ( empty( $q['orderby'] ) ) {
			...
			if ( isset( $q['orderby'] ) && ( is_array( $q['orderby'] ) || false === $q['orderby'] ) ) {
				...
				$orderby = "{$wpdb->network_posts}.post_date " . $q['order'];
			}
		} elseif ( 'none' == $q['orderby'] ) {
			$orderby = '';
		} elseif ( $q['orderby'] == 'post__in' && ! empty( $post__in ) ) {
			$orderby = "FIELD( {$wpdb->network_posts}.ID, $post__in )";
		} elseif ( $q['orderby'] == 'post_parent__in' && ! empty( $post_parent__in ) ) {
			$orderby = "FIELD( {$wpdb->network_posts}.post_parent, $post_parent__in )";
		} elseif ( $q['orderby'] == 'post_name__in' && ! empty( $post_name__in ) ) {
			$orderby = "FIELD( {$wpdb->network_posts}.post_name, $post_name__in )";
		} else {
			$orderby_array = array();
	...
	protected function parse_orderby( $orderby ) {
		...
		switch ( $orderby ) {
			...
			case 'menu_order':
			case 'comment_count':
				$orderby_clause = "{$wpdb->network_posts}.{$orderby}";
				break;
			case 'rand':
				$orderby_clause = 'RAND()';
				break;
			...
			default:
				if ( array_key_exists( $orderby, $meta_clauses ) ) {
					...
					$orderby_clause = "{$wpdb->network_posts}.post_" . sanitize_key( $orderby );
				}
