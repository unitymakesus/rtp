...
if(isset($objects[$object])){
  ...
  */
  ...
  $str .= '<li style="margin:0; padding:0;" data-default="'.$default.'" data-value="'.htmlentities($value['value']).'" data-label="'.htmlentities($value['value']).'">'.htmlentities($value['value']).' '.$default_html.'</li>';
  }
  $str .= '</ul>';
  return $str;
  }
  ...
  */
  ...
  $str='<div><b>'.__('No Options Found','gravity-forms-salesforce-crm').'</b></div>';    
  }
  /// $str = str_replace(array("\n", "\t", "\r"), '', str_replace("'", "\'", $str));
...
public function editor_js() {
  ...
  // Reset the fields
  $('.<?php echo $this->id ?>_map_type, .<?php echo $this->id ?>_map_enabled').attr('checked', false);
  $('.<?php echo $this->id ?>_field_list').html('');
   $(".<?php echo $this->id ?>_object").val(""); 
  if(typeof field.crm_map_enabled !== "undefined" && field.crm_map_enabled=="<?php echo $this->id ?>") { ///console.log(field.crm_map_enabled);
  ...
  div.find(".<?php echo $this->id ?>_map_type[value='"+map_type+"']").prop('checked',true).trigger('change'); 
  setTimeout(function(){
  toggle_choices(obj);   
  },1000);     
  
 /// div.find(".<?php echo $this->id ?>_object").val(field.crm_object).trigger('change');
 var account_sel=div.find(".<?php echo $this->id ?>_account");
...
account_sel.val(crm_account);
 vx_load_account(account_sel,crm_object);
  ...
  $('.<?php echo $this->id ?>_map_enabled').trigger('change'); 
  //select object
  
  });
  $('.<?php echo $this->id ?>_map_enabled').unbind('change click').on('click change', function () {
  
  crm_checkbox($(this));
  });
  $('.<?php echo $this->id ?>_map_type').unbind('change click').on('click change', function () {
  var val=$(this).val();
  ...
  $('.crm_setting').on('click','.<?php echo $this->id ?>_map_field',function(e){
  ...
  //.unbind('change')
  $('.<?php echo $this->id ?>_account').on('change',function(e){
vx_load_account($(this),field.crm_object);
  });

$(document).on('change','.<?php echo $this->id ?>_object',function(e){ 
...
//  SetFieldProperty('crm_object',object); 
  ...
  div.find(".<?php echo $this->id ?>_objects").html(res);
  div.find("#<?php echo $this->id ?>_field_group").slideDown(); 
  sel_radio_option();
  ...
  var form_id=div.find(".crm_form_id").val();
  var action=elem.attr('id');
var account=div.find('.<?php echo $this->id ?>_account').val();
  ...
  div.find(".<?php echo $this->id ?>_field_list").html(res);
  div.find("#<?php echo $this->id ?>_field_group").slideDown();
  ///div.find('._field_list').trigger('load');
sel_radio_option();
  //eval(res);
...
//  toggle_choices(check_box);  
 ...
 function sel_radio_option(){
  ...
  jQuery("#<?php echo $this->id ?>_map_field_"+field.crm_option).attr('checked',true).trigger('change');
  /* var field = GetSelectedField();
  UpdateFieldChoices(field.type);
  LoadFieldChoices(field);*/   
  }
 ...
 <div class="<?php echo $this->id ?>_objects"></div>
  </div>
 ...
 public function object_area($id,$object){
  ...
  <div class="<?php echo $this->id ?>_field_list">
  <?php
