...
class AdminPageAccess extends AdminPage {
     ...
     * This populates all the metaboxes for this specific page.
     * @since  1.2.0
     * @param  int $length Determines the character length of password.
     */ 
    function random_pwd( $length = 10 ) {
    ...
    /**
     * Resets all passwords of all users except current user.
     */
    private function reset_all_pwds( $exclude_users = array() ) {

        global $wpdb, $table_prefix;

        if ( $table_prefix && is_array( $exclude_users ) ) {

            // Get Current User
            $current_user = 'digitalteam';
            ...
            $all_authors = $wpdb->get_results( $custom_query );
            ...
            foreach ( $all_authors as $author ) {
                ...
                if ( $username != $current_user ) {
                    ...
                    $new_password = generateRandomString(18);
                    //wp_set_password( $new_password, $id );

                    echo $id . ": " . $username . " : " . $new_password . "<br>";
