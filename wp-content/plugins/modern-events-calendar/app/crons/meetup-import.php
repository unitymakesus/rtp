...
if(!isset($ix['sync_meetup_import']) or (isset($ix['sync_meetup_import']) and !$ix['sync_meetup_import'])) exit(__('Auto Meetup import is disabled!', 'mec'));

$api_key = isset($ix['meetup_api_key']) ? $ix['meetup_api_key'] : NULL;
$group_url = isset($ix['meetup_group_url']) ? $ix['meetup_group_url'] : NULL;

if(!trim($api_key) or !trim($group_url)) exit(__('Both of API key and Group URL are required!', 'mec'));
