...
class MEC_feature_gateways extends MEC_base
{
    ...
    public function do_free_booking()
    {
        ...
        if(!wp_verify_nonce(sanitize_text_field($_POST['_wpnonce']), 'mec_transaction_form_'.$transaction_id)) $this->main->response(array('success'=>0, 'code'=>'NONCE_IS_INVALID', 'message'=>__('Request is invalid!', 'mec')));
...
class MEC_gateway_stripe extends MEC_gateway
{
    ...
    public function do_transaction($transaction_id = NULL)
    {
        ...
        // Verify that the nonce is valid.
        if(!wp_verify_nonce(sanitize_text_field($_GET['_wpnonce']), 'mec_transaction_form_'.$transaction_id)) $this->response(array('success'=>0, 'code'=>'NONCE_IS_INVALID', 'message'=>__('Request is invalid!', 'mec')));
        ...
        $ticket_ids = '';
        foreach($attendees as $attendee) $ticket_ids .= $attendee['id'].',';
        $ticket_ids = ','.trim($ticket_ids, ', ').',';

        $user_id = $this->register_user($main_attendee);

        $book_subject = $name;
        $book_id = $this->book->add(array('post_author'=>$user_id, 'post_type'=>$this->PT, 'post_title'=>$book_subject, 'post_date'=>$date), $transaction_id, $ticket_ids);
