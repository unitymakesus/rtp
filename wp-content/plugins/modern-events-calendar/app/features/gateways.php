...
class MEC_gateway extends MEC_base implements MEC_gateway_interface {
	...
	public function get_request_string( $vars ) {
		...
		foreach ( $vars as $var => $val ) {
			$string .= '&' . $var . '=' . urlencode( stripslashes( $val ) );
		}
...
class MEC_gateway_stripe extends MEC_gateway {
	...
	public function __construct() {
		...
		if ( $this->enabled() and ! is_admin() ) {
			$this->factory->action( 'wp_enqueue_scripts', array( $this, 'frontend_assets' ) );
		}
	}
	...
	public function frontend_assets() {
		...
		if ( $stripe_js ) {
			wp_enqueue_script( 'mec-stripe', 'https://js.stripe.com/v2/' );
		}
	}
	...
	public function do_transaction( $transaction_id = null ) {
		...
		// Stripe Token
		$token = isset( $_GET['stripe_token'] ) ? sanitize_text_field( $_GET['stripe_token'] ) : '';
	...
	public function options_form() {
		...
		<div class="mec-form-row">
			<label>
				...
				<input onchange="jQuery('#mec_gateways<?php echo $this->id(); ?>_container_toggle').toggle();" value="1" type="checkbox" name="mec[gateways][<?php echo $this->id(); ?>][status]" 
																 <?php
			...
			<div class="mec-form-row">
				...
				<div class="mec-col-4">
					...
					<span class="mec-tooltip">
						<div class="box top">
							<h5 class="title"><?php _e('Comment', 'mec'); ?></h5>
							<div class="content"><p><?php esc_attr_e('HTML allowed.', 'mec'); ?><a href="https://webnus.net/dox/modern-events-calendar/woocommerce/" target="_blank"><?php _e('Read More', 'mec'); ?></a></p></div>
						</div>
						<i title="" class="dashicons-before dashicons-editor-help"></i>
					</span>						
				</div>
				...
				<div class="mec-col-4">
					...
					<span class="mec-tooltip">
						<div class="box top">
							...
							<div class="content"><p><?php esc_attr_e('It applies only to the orders that are related to MEC.', 'mec'); ?><a href="https://webnus.net/dox/modern-events-calendar/woocommerce/" target="_blank"><?php _e('Read More', 'mec'); ?></a></p></div>
						</div>
						<i title="" class="dashicons-before dashicons-editor-help"></i>
					</span>						
				</div>
