...
if ( isset( $settings['search_bar_ajax_mode'] ) && $settings['search_bar_ajax_mode'] == '1' ) : 
    ...
    <div class="mec-ajax-search-result">
        <div class="mec-text-input-search">
            ...
            <input type="text" placeholder="'.__('Please enter at least 3 characters' , 'mec').'" value="" id="keyword" name="keyword" />
        </div>
        <div id="mec-ajax-search-result-wrap"><div class="mec-ajax-search-result-events">'.__('Search results will show here' ,'mec').'</div></div>
    </div>';
else: 
    ...
    $output .= '<input class="mec-search-bar-input" id="mec-search-bar-input" type="submit" alt="'.esc_html__('Search', 'mec').'" value="'.esc_html__('Search', 'mec').'" /><input type="hidden" name="post_type" value="mec-events">';
endif;
