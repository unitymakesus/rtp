...
public function plugin_api_setup(){
  ...
  */
  ...
  $linkText = __('Check for updates','gravity-forms-salesforce-crm');
...
$message='<a href="'.$this->settings_link.'" title="Register">Register</a> your copy of '.$this->title.' to get access to automatic updates. Need a license key? Purchase one <a href="'.$product_url.'" target="_blank"  title="Purchase License">here</a>';
  ...
  $message=$this->lic_msg= sprintf(__("Your License Key expired. %sRenew it Now%s",'gravity-forms-salesforce-crm'),'<a href="'.$lic_url.'" target="_blank">','</a>');
  ...
  */
  ...
  if($_GET['vx_update_check_result'] == "update_available"){
    $msg=sprintf(__('%s Update is available','gravity-forms-salesforce-crm'),"<b>".$this->title.".</b>");   
  }else{
   $msg=sprintf(__('%s Update is not available','gravity-forms-salesforce-crm'),"<b>".$this->title.".</b>"); 
  }
  $this->screen_msg($msg);
  } 
...
public function add_section_wc($tabs){
    ...
    $tabs["vxc_license"]=__('License Key','gravity-forms-salesforce-crm');
    }
    return $tabs;
}
/**
...
public  function first_page($page_added,$form_tag=''){ 
  ...
  $message= __("Error while installing Plugin",'gravity-forms-salesforce-crm');
  $class="error";
  if(isset($vx_res['status']) && $vx_res['status'] == "ok"){
  $valid=true;
  $message=__("Plugin installed successfully",'gravity-forms-salesforce-crm');
  $class="updated";
  ...
  ?>
  <h2>
  <?php _e('Redirecting, Please Wait','gravity-forms-salesforce-crm'); ?>
  </h2>
  <?php
  if($message) {
  ...
  ?>
  <h2>
  <?php echo sprintf(__('Activate %s','gravity-forms-salesforce-crm'),$this->title); ?>
  </h2>
  <?php if($message) {
  echo "<div class='fade below-h2 {$class}'>".wpautop($message)."</div>";
  }
  ...
  <p style="text-align: left;"> <?php echo sprintf(__("Don't have a license key? Purchase one %s here %s.", 'gravity-forms-salesforce-crm'), "<a href='".$this->url."' target='_blank'>" , "</a>") ?> </p>
  <table class="form-table">
  <tr>
  <th scope="row"><label for="vx_key">
  <?php _e("License Key", 'gravity-forms-salesforce-crm'); ?>
  </label>
  </th>
  <td><input type="text" size="75" name="lic_key" id="vx_key" />
  <br/>
  <?php _e("Enter Your License Key", 'gravity-forms-salesforce-crm') ?></td>
  </tr>
  <tr>
  <td colspan="2" ><input type="submit" name="<?php echo $this->id ?>_install" class="button-primary" value="<?php _e("Save Settings", 'gravity-forms-salesforce-crm') ?>" /></td>
  </tr>
  </table>
  <div> </div>
  <?php
...
$this->screen_msg($msg['msg'],$msg['class']);
  ...
  <h2>
  <?php esc_html_e("License Key", 'gravity-forms-salesforce-crm') ?>
  </h2>
  <p><?php echo sprintf(__("If you don't have a license key, you can get from %shere%s.",'gravity-forms-salesforce-crm'),'<a href="'.$this->url."?vx_product=".$this->sku.'" target="_blank" title="'.__('Get License Key','gravity-forms-salesforce-crm').'">','</a>'); ?></p>
  <table class="form-table">
  <tr>
  ...
  <input type="password" style="width: 100%;" class="crm_text" id="vx_license_key" name="vx_lic_key" placeholder="<?php _e('Enter License Key','gravity-forms-salesforce-crm') ?>" value="<?php echo $lic_key ?>">
  </div><div style="display: table-cell;">
  <a href="#" style="margin: 0 0 0 10px; vertical-align: baseline; text-align: center; width: 110px" class="button vx_toggle_key ddd" id="vx_toggle_key" title="<?php _e('Toggle License Key','gravity-forms-salesforce-crm'); ?>">Show Key</a>
  ...
  <?php
      ...
      ?>
  <div style="color:rgb(0, 132, 0); padding-bottom: 8px; margin-top: 8px;"><i class="fa fa-check"></i> <?php echo sprintf(__("License Key is Valid - %s",'gravity-forms-salesforce-crm'),'<code>'.date('F d, Y h:i:s A',$lic_time).'</code>'); ?></div>
  <?php
  }
                      ?></td>
  </tr>
  <tr>
  <th colspan="2" ><input type="submit" name="<?php echo $this->id."_key"; ?>" class="button-primary" title="<?php _e('Save License Key','gravity-forms-salesforce-crm'); ?>" value="<?php _e("Save License Key", 'gravity-forms-salesforce-crm') ?>" /></th>
  </tr>
  </table>
  <?php
       if(empty($form_tag)){ 
